...
func (s *Service) ListDrills(ctx context.Context) ([]domain.Drill, error) {
	if s.drillListCache != nil {
		return append([]domain.Drill(nil), s.drillListCache...), nil
	}
	drills, err := s.store.ListDrills(ctx)
	if err != nil {
		return nil, err
	}
	s.drillListCache = append([]domain.Drill(nil), drills...)
	return drills, nil
}
